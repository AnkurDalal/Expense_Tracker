      ...
      <div className='container-custom'>
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'} mb-2`}>Financial Reports</h1>
              <p className={`${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Detailed analytics and insights for your finances</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className={`p-6 rounded-2xl shadow-lg ${
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`${isDarkMode ? 'text-green-200' : 'text-green-100'}`}>All income sources</p>
              </div>
          ...
          <div className={`p-6 rounded-2xl shadow-lg ${
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`${isDarkMode ? 'text-red-200' : 'text-red-100'}`}>All expense categories</p>
              </div>
              <div className="text-right">
                <p className="text-2xl font-bold">₹{addThousandSeparator(totalExpense)}</p>
                <p className={`${isDarkMode ? 'text-red-200' : 'text-red-100'} text-sm`}>INR</p>
              </div>
            </div>
          </div>
          ...
          <div className={`p-6 rounded-2xl shadow-lg ${
            ...
            isDarkMode ? 'from-green-600 to-emerald-700' : 'from-green-500 to-emerald-600' : 
            isDarkMode ? 'from-red-600 to-pink-700' : 'from-red-500 to-pink-600'
          }`}>
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-lg font-semibold mb-1">Net Savings</h3>
                <p className={`${isDarkMode ? 'text-blue-200' : 'text-blue-100'}`}>Income - Expenses</p>
              </div>
              <div className="text-right">
                <p className="text-2xl font-bold">₹{addThousandSeparator(netSavings)}</p>
                <p className={`${isDarkMode ? 'text-blue-200' : 'text-blue-100'} text-sm`}>INR</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Trend Analysis */}
        <div className={`rounded-2xl shadow-lg p-6 border mb-8 ${
          isDarkMode ? 'bg-gray-800 border-gray-700/50' : 'bg-white border-gray-100'
        }`}>
          <h2 className={`text-xl font-semibold mb-4 flex items-center gap-2 ${
            isDarkMode ? 'text-white' : 'text-gray-900'
          ...
          <div className={`rounded-2xl shadow-lg border overflow-hidden ${
            ...
            <div className={`p-6 border-b ${
              ...
              <h2 className={`text-xl font-semibold ${
                isDarkMode ? 'text-white' : 'text-gray-900'
              }`}>Top Income Sources</h2>
            </div>
            <div className="p-6">
              {incomeChartData.slice(0, 5).map((item, index) => (
                <div key={index} className={`flex items-center justify-between py-3 ${
                  isDarkMode ? 'border-b border-gray-700/50' : 'border-b border-gray-100'
                } last:border-b-0`}>
                  <div>
                    <p className={`font-medium ${
                      isDarkMode ? 'text-white' : 'text-gray-900'
                    }`}>{item.name}</p>
                    <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'} text-sm`}>{((item.value / totalIncome) * 100).toFixed(1)}% of total</p>
                  </div>
                  <p className={`font-semibold ${
                    isDarkMode ? 'text-green-400' : 'text-green-600'
                  }`}>₹{addThousandSeparator(item.value)}</p>
                </div>
              ))}
            </div>
          </div>
              ...
              {expenseChartData.slice(0, 5).map((item, index) => (
                <div key={index} className={`flex items-center justify-between py-3 ${
                  ...
                  <div>
                    ...
                    <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'} text-sm`}>{((item.value / totalExpense) * 100).toFixed(1)}% of total</p>
                  </div>
                  <p className={`font-semibold ${
